...
COLORREF colors[256] = { RGB(255,138,180),RGB(252,145,196),RGB(255,144,194),
                         RGB(255,144,194),RGB(255,143,195),RGB(255,140,190),RGB(255,142,193) };
...
void creat_data()
{
    ...
    for (double size = 0.1, lightness = 1.5; size < 20; size += 0.1)
    {
        ...
        for (int i = 0; i < quantity; ++i)
        {
            if (success_p > creat_random(0, 100) / 100.0)
            {
                ...
                points[index].color = RGB(GetRValue(color) / lightness,
                    GetGValue(color) / lightness, GetBValue(color) / lightness);
    ...
    for (int frame = 0; frame < frames; ++frame)
    {
        ...
        for (index = 0; index < points_size; ++index)
        {
            ...
            double distance_increase = -0.0009 * distance * distance
                + 0.35714 * distance + 5;
